    ...
    def build(
        ...
        **params,
        ...
        for index, f in enumerate(*filters):
            ...
            print((nested_path, mapping_type))
            ...
            if len(nested_path) >= 1:
                keyword_path = _get_keyword_path(mapping_type, property_path)
                if keyword_path:
                    filter_or_must_not = (
                        "filter"
                    ...
                    """
                    _filter, must, must_not, script_score = _create_nested_bool_query(
                        f,
                        nested_path,
                        mapping_type,
                        keyword_path,
            ...
            # nested_path = [] => (i.e mapping_type == Text, Date, Keyword, ...) - nested_path empty
                ...
                if mapping_type is None:
                    ...
                    keyword_path = _get_keyword_path(
                        None,
                        property_path,
                ...
                if keyword_path:
                    ...
                    """
                    (_filter, must, must_not, script_score) = _create_non_nested_query(
                        f,
                        mapping_type,
                        keyword_path,
                        property_path,
...
def _look_up_known_parent_paths(f, last_path, property_path, m):
    ...
    ):  # to cope with paths.type.id => to externalise
...
def _wrap_in_script_score_query(query, script, includes=None, excludes=None):
    ...
    # Catch non supported combination of nested query and non nested ones
...
def _create_bool_query_template(
    ...
    if (
        ...
        if nested_path:
            ...
            script_score = _wrap_in_script_query(keyword_path, filter.value)
        ...
        if nested_path:
            ...
            query = _wrap_in_non_nested_query_query(
                keyword_path, term_or_match, filter.value
            )
...
def _wrap_in_nested_bool_query(
    path, filter_or_must_or_mustnot, filter_type, filter_value
):
    ...
    return _wrap_in_nested_query(
        ...
        query=elasticsearch_dsl.query.Q(
            "bool",
            **{
                filter_or_must_or_mustnot: elasticsearch_dsl.query.Q(
...
def _create_non_nested_query(
    ...
    # Range query should not be run on Text, Boolean and Keyword fields because of expensive query
    if (
        filter.operator in elasticsearch_operator_range_map.keys()
        and not isinstance(mapping_type, elasticsearch_dsl.Text)
        and not isinstance(mapping_type, elasticsearch_dsl.Boolean)
        ...
        must = _wrap_in_non_nested_query_query(
            "field", "exists", keyword_path
        )  # otherwise script_score will fail

    else:  # text => match query
        query = _wrap_in_non_nested_query_query(
            keyword_path, term_or_match, filter.value
        )
